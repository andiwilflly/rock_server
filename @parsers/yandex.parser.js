...
async function parsePage(cluster, group, album) {
    ...
    try {
        await cluster.execute(null, async ({ page, name }) => {
            console.log(`✨ YANDEX PARSER | cluster.task created`);
            ...
            if(!artistLink) return RESULT = { source: 'yandex', error: `No such group: ${group}` };
            ...
            if(!albumLink) return RESULT = { source: 'yandex', error: `No such album: ${album}` };
